...
const PATCHES = [
  {
    ...
    replace: `要件定義の精度が、これまで以上に成果物の品質を左右します。</p>
<p>この「仕様の精度がそのまま成果物になる」性質に向き合う進め方が仕様駆動開発（SDD）です。実装させる前にどこまで決めておけばよいかは${LINK}でまとめています。</p>`,
  },
  {
    ...
    replace: `<li><a href="/column/user-story-template-examples">User Storyの書き方</a></li><li>${LINK}</li></ul>`,
  },
  {
...
for (const p of PATCHES) {
  ...
  console.log(
    `  applied. リンク存在: ${after.content.includes('/knowledge/spec-driven-development')}`
  );
}
