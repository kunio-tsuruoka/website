...
export const services: ServiceDetail[] = [
  ...
  },
  {
    ...
    solutions: [
      ...
      },
      {
        title: 'RAG（検索拡張生成）構築',
        description:
          '社内ドキュメント・FAQ・マニュアルを取り込み、回答の正確性・最新性を担保するRAGパイプラインを構築。Embeddingモデル選定からベクトルDB（Milvus / pgvector等）、グラフDB（Neo4j）まで対応',
  ...
  },
  {
    ...
    caseStudies: [
      ...
      {
        title: '不動産管理会社（従業員500名）の契約文書RAG検索',
        challenge:
          '賃貸契約書、管理委託契約書、修繕記録など数千件の契約関連文書がPDFで保存されていた。オーナーや入居者からの問い合わせ対応で「この物件の契約条件は？」「過去の修繕履歴は？」といった質問に、毎回PDFを手動で探して確認する作業が発生していた。',
        solution:
          'PDF文書をOCR+テキスト抽出し、物件番号・契約種別・日付などのメタデータを付与したRAGシステムを構築。物件番号で絞り込んだうえで自然文検索ができる設計とし、回答に該当契約書のページ番号を併記する仕組みを組み込んだ。',
        results: [
          '問い合わせ対応の文書検索時間を平均15分から2分に短縮',
          '物件番号による絞り込みで誤った契約書の参照を防止',
          '契約更新時の条件確認作業を大幅に効率化',
        ],
      },
    ],
    ...
    faq: [
      ...
      {
        question: 'パッケージ型のAI検索ツールと何が違いますか？',
        answer:
    ...
    additionalSections: [
      {
        ...
        paragraphs: [
          ...
          'たとえば「請求システムの統合で問題が起きているが、誰に聞けばいいか？」と質問した場合、通常のRAGは請求に関する文書、統合に関する文書、担当者リストをそれぞれ見つけてきます。しかし「田中さんがその統合を担当していて、佐藤さんが直近のバグ対応をした」という関連付けはできません。文書が別々に存在しているからです。',
          'GraphRAGは、文書を孤立した断片として保存するのではなく、人・プロジェクト・システム・出来事の間の関係をグラフ構造でマッピングします。質問を受けると、まずベクトル検索で関連する起点を見つけ、そこからグラフをたどって「つながり」を理解します。',
          '実際に構築した要件管理システムでは、「先月のDB設計変更で影響を受けた機能は何か、誰が対応したか」のような質問に対して、変更イベント・影響を受けた機能・担当者・関連する議事録の間のつながりをたどり、一つの回答として返せるようになりました。',
          'セットアップは通常のRAGより手間がかかり、関係性を正しく抽出するためのデータ品質も求められます。しかし「情報のつながり」が重要な業務ナレッジベースでは、その投資に見合う価値があります。検索できるだけのAIと、業務の文脈を理解して推論できるAIの違いです。',
        ],
      },
    ],
    relatedColumns: [
