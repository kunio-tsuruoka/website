...
export const services: ServiceDetail[] = [
  ...
  },
  {
    ...
    faq: [
      ...
      },
      {
        ...
        answer:
          '文書の量・形式・連携先によって大きく変動します。初回ヒアリングで要件を確認した上で、個別にお見積りします。初期費用0円のゼロスタート（検証用プロトタイプ）から始め、実物を確認してから有料のPoCへ進むことも可能です。',
      },
      {
  ...
  },
  {
    ...
    faq: [
      ...
      },
      {
        ...
        answer:
          '対象文書の量・形式・連携先によって大きく変動します。初回ヒアリングで要件を確認した上で、個別にお見積りします。初期費用0円のゼロスタート（検証用プロトタイプ）から始め、実物を確認してから有料のPoCへ進むことも可能です。',
      },
      {
  ...
  },
  {
    ...
    faq: [
      ...
      },
      {
        question: '導入期間と費用の目安を教えてください',
        answer:
          '帳票の種類・枚数・連携先によって大きく変動します。初回ヒアリングで実際の帳票サンプルを拝見した上で、個別にお見積りします。初期費用0円のゼロスタート（検証用プロトタイプ）から始め、実物を確認してから有料のPoCへ進むことも可能です。',
      },
      {
  ...
  },
  {
    ...
    seoTitle: 'MVP開発・PoC開発の外注｜初期費用0円のプロトタイプから新規事業を検証し、本開発へ',
    seoDescription:
      'MVP開発・PoC開発を外注するなら。課題・アイデア→無料プロトタイプ→PoC→MVP→本開発の標準プロセスで、新規事業・DXを小さく検証してから本番化します。自社開発のPM基盤「PM on Rails」で仮説・要件・受入基準を構造化し、AIエージェントで高速に実装・修正。初期費用0円・最短2週間で投資判断の材料を出します。',
    description:
      'アイデアや新規事業を、大きく作る前に小さく検証するためのMVP・PoC・プロトタイプ開発です。検証項目の設計から入り、初期費用0円・最短数日で動くものを作り、投資判断の材料を揃えます。',
    longDescription:
      'MVP・PoC・プロトタイプ開発でよくある失敗は、「とりあえず動くデモ」を作って満足してしまい、本開発や投資判断につながらないことです。Beekleは「課題・アイデア→無料プロトタイプ→PoC→MVP→本開発」を標準プロセスとして持ち、まず「何をもって成功とするか（検証したい仮説と評価基準）」を一緒に定義します。そのうえで初期費用0円・最短数日で検証用プロトタイプを作り、実物を確認してから有料のPoC（最短2週間）で実際の業務やユーザーで仮説を確かめます。この反復を支えているのが、自社開発のPM基盤「PM on Rails」です。仮説を要件・受入基準まで構造化し、その仕様をAIエージェントによる実装へつなぐので、「作って、確かめて、修正する」を速度を落とさず繰り返せます。検証で得た学びは同じチームがそのまま本開発へ引き継ぐため、PoCがデモ止まりになりません。生成AIを使ったPoCにも対応します。',
    painPoints: [
      {
    ...
    solutions: [
      ...
      {
        title: '初期費用0円・最短数日の検証用プロトタイプ／最短2週間のPoC',
        description:
          'ゼロスタートとして初期費用0円・最短数日で検証用プロトタイプを作成。実物を確認してから、有料のPoC（最短2週間）で実際の業務やユーザーで仮説を検証し、投資判断の材料を揃えます。',
    ...
    features: [
      ...
      {
        title: '初期費用0円の検証用プロトタイプ／最短2週間のPoC',
        description:
          '検証用プロトタイプは初期費用0円で、予算を確保する前に着手できます。実物を確認してから、有料のPoC・MVP・本開発へ進むかを判断できます。',
      },
      {
    ...
    faq: [
      ...
      },
      {
        ...
        answer:
          '無料なのは検証用プロトタイプまでで、Beekleのゼロスタートでは初期費用0円で作成できます。その先のPoCは有料で、最短2週間が目安です。検証範囲によって変わるため、まず確かめたい仮説をうかがって設計します。',
      },
      {
    ...
    additionalSections: [
      {
        ...
        subtitle: '無料プロトタイプ→PoC→MVP→本開発。どの段階からでも始められます',
        paragraphs: [
          '課題やアイデアの段階から、無料プロトタイプ、PoC、MVP、本開発へ。Beekleはこの流れを標準プロセスとして持ち、各段階の終わりに「次へ進むか、やめるか」を判断できる材料を出します。',
        ],
        cards: [
          {
          ...
          {
            title: 'STEP 2：無料プロトタイプ（ゼロスタート）',
            description:
              '初期費用0円・最短数日で、コア機能に絞った動くものを作ります。触ってから、続きに進むかを判断できます。',
          },
          {
            title: 'STEP 3：PoC',
            description:
