export const aiServicePageConfig = {
  ...
  'requirements-definition-support': {
    ...
    heroLead:
      'PM on Railsは、もともと社内で回していた進め方をAIエージェントで動くようにした自社基盤です。会話を要求・受入条件・実装タスクまでつなぐので、要件定義が長引かず、AIが迷わず実装できます。',
    primaryOutcome:
    ...
    rtb: {
      ...
      note: {
        ...
        description:
          'もともと社内で回していた開発の進め方を、AIエージェントで動くようにしたものです。新しく持ち込んだ手法ではなく、これまでの案件で使ってきたやり方をそのまま仕組みにしています。要求、要件、ユーザーストーリー、受入条件、実装タスク、確認結果を複製せず関係でつなぎ、いまもBeekleの開発現場で使っています。相談のあと、会話がどの形で残るかを一緒に見られます。',
