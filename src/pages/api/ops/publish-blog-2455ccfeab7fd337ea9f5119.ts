...
const DESCRIPTION = '正式公開直前に止まったシステムを引き継ぎ、PM on RailsとCursorエージェントに人間の設計判断を組み合わせ、3日強で要件を整理し、約2週間で開発59％まで進めた実案件の記録です。';
...
## 仕組みだけで、同じ結果が出るわけではありません

ここも誤解されたくないところです。PM on RailsとCursorエージェントを用意すれば、誰でも同じ速度と品質になるわけではありません。

今回、3日強で要件を再整理できたのは、資料を順番に入力したからだけではないです。どの資料を正しいものとして扱うか、何が矛盾しているか、どこをお客さんへ確認するか、決済、権限、例外処理のどこで事故が起きやすいかを判断しました。そこはPM on Railsが自動で決めたわけではなく、私の要件定義や設計の経験を使っています。

AIは、人の能力を均一にするというより、持っている能力を増幅する道具に近いと思っています。

要件が曖昧なままなら、曖昧な実装が速く増えます。設計を間違えれば、間違った方向へ速く進みます。逆に、設計の勘所が分かる人が使えば、調査、実装、確認をAIへ任せ、その分だけ重要な判断へ時間を使えます。

今回の速さは、仕組みだけの成果でも、私一人だけの成果でもありません。PM on Rails、Cursorエージェント、開発担当者と、私の要件定義、設計、確認の経験を組み合わせた結果です。

仕組みで無駄を減らし、AIで実行を加速する。でも、何を作るべきか、どこが危ないか、何を完成とするかを決める力までは消えません。むしろAIを使えるようになるほど、その判断の差が速度と品質の差になって出る気がします。
