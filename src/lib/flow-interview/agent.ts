...
- 相手が「わからない」「特に無い」「時間がかかる」など曖昧・短い回答をしたら、それ以上同じ項目を掘らず次の未確認項目へ移る
...
export function computeCoverage(d: FlowDiagram): Coverage {
  ...
  else if (!hasTitle) nextAspect = 'この業務の名前';
  else if (stepCount < 3) nextAspect = '開始から完了までの大まかな手順';
  else if (laneCount < 1) nextAspect = '関わる担当・部署（登場人物）';
  else if (!hasStart || !hasEnd) nextAspect = '業務の開始のきっかけと、完了の状態';
  // それ以外は一通り埋まった → 完成可能（nextAspect=null）
