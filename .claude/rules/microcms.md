...
# 新マーカー導入時のデプロイ順序: コード先 → CMS後（2026-06-19 やらかし）

`column-visuals.ts` に新しい `{{MARKER}}` を追加して、MicroCMS 本文にそのマーカーを挿入する施策では、**必ずレンダリング側コードを本番デプロイしてから MicroCMS を PATCH する**。

## why
MicroCMS の PATCH は **即・本番反映**される（プレビュー段階が無い）。一方 `renderColumnVisuals` の新マーカー対応コードはマージ→Cloudflareデプロイまで本番に出ない。順序を逆にすると、その間 **本番の対象ページにリテラル `{{ESTIMATE_CONSULT}}` 等がそのまま表示される**（renderColumnVisuals は未知マーカーを素通しするため）。

2026-06-19、買い手ページの意図別相談CTA施策で、コード未マージのまま先に7ページを PATCH → 本番に literal マーカーが数分間露出。PR #52 を急ぎマージして解消。

## how to apply
1. まずコード（マーカー定義＋renderColumnVisualsのループ追加）を PR→マージ→Cloudflareデプロイ完了まで待つ
2. デプロイ確認後に `patch-*.mjs --apply` で MicroCMS にマーカー挿入
3. やむなく逆順になったら、コードPRを最優先でマージしてデプロイ窓を最小化する
4. 既存マーカー（CONTACT_CTA 等、既にコード対応済み）の挿入だけなら順序自由

# MicroCMS は見出しidをテキストから自動採番する（重複見出し→重複id）

`columns` の `<h2>/<h3>` の `id="h<hash>"` は **MicroCMS のサニタイザが見出しテキストから自動生成**する。同一文言の見出し（例「向くケース」「注意点」を選択肢ごとに3回）があると **必ず同じidになり、重複id（HTML妥当性違反）になる**。

- content を PATCH して id を手で書き換えても**サニタイザに元の自動idへ戻される**ため、content側ではidを一意化できない。
- 真に一意化するには**見出しの文言自体を変える**しかない（例「1社固定が向くケース」）。
- ただし目次アンカー（`href="#..."`）がその記事に無ければ**実害なし**（重複idはHTML妥当性のみの問題）。修正優先度は低い。

検出: `grep -oE 'id="h[0-9a-f]+"' | sort | uniq -c | sort -rn` で2以上が重複。アンカー参照は `grep -oE 'href="#h[0-9a-f]+"'` が0なら無害。

事例 2026-07-01: `llm-selection-strategy` で「向くケース/注意点」が各3回→各重複id。アンカー0件で無害のため見出し改名は保留。

# publish-drafts.mjs は claudedocs/drafts/ 内の未公開ドラフトを「全部」公開する。保留ドラフトは必ずフォルダ外へ退避

`scripts/publish-drafts.mjs --apply` は `claudedocs/drafts/` の .md/.html を全走査し、**同slugのcolumnが未存在なら全部 client.create で即公開**する（特定ファイルだけ公開する引数は無い）。「1本だけ公開」のつもりで走らせると、そのフォルダに置いた**保留中ドラフトも巻き添えで公開**される。

- 事故 2026-07-05: 3本公開のつもりが `created: 7/25`。市場調査で0需要と判明し「保留」にしたB4-B7(reduce-phone-inquiries/why-faq-not-used/after-hours-inquiry-response/compatibility-table-beyond-excel)がdraftsに残っていて一緒に公開された。→ `client.delete({endpoint:'columns',contentId})` で4本削除して復旧、ドラフトは `claudedocs/drafts-held/` へ退避。
- 予防: **公開したくないドラフトは `claudedocs/drafts/` に置かない**。保留は `claudedocs/drafts-held/`(publish対象外)へ。公開前に必ず `node scripts/publish-drafts.mjs --dry` で「今どれが created 予定か」を確認する。dryの created 件数＝公開される本数。
- 削除は reversible: ローカルにドラフトが残っていれば slug 指定で再 create すればURLごと復活する。
- 関連: [[project_llmo_content_map]], microcms.md(publish-drafts の status:draft は実際は即公開)。

# getQAs も全件ページネーション必須（qasが100件超で /qa とFAQPage schemaから脱落）

`src/lib/microcms.ts` の `getQAs` は `limit:100` 固定・ページネーション無しだった。qas総数が100を超えると、`/qa`(src/pages/qa.astro が `getQAs(undefined)` を使用)の一覧とFAQPage構造化データが**100件で頭打ち**になり、order末尾のQ&Aが黙って脱落する。getColumns と同型のバグ([[project]]の「getColumns は全件ページネーション必須」と同根)。

- 2026-07-05: AIサポート系Q&Aを24問追加(143→167件)して発覚。追加前の143件時点で既に100超え=以前から取りこぼしていた。offsetでtotalCountまでループ取得に修正済み(commit fix(qa): getQAsを全件ページネーション化)。
