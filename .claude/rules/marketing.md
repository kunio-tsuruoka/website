...
## 2026-07-18 追加実績（匿名化マッピング／Beekleポートフォリオ）

`/case-studies` の `allCases`（iroAI直後）に6件、`service.ts` の web-mobile-development `caseStudies` に matchin詳細版を登録。社名・プロダクト名・ドメインは非公開。**受託案件は掲載許諾が未取得**（zutool=発注元、oem=発注元。伊藤忠/NTT同様に社名を出さない前提で匿名掲載）。

- **商用モバイルアプリのFlutter移行**（実体: `hedgehog-medtech/zutool-flutter-v1`、頭痛・気圧アプリ「頭痛ーる」、発注元=ヘッジホッグ・メドテック、**受託・許諾未取得**）。公開表記は気圧/頭痛/生理周期等のドメイン語を伏せ「商用モバイルアプリ」。IDR86・E2E19は実測。「ユースケース駆動で開発」は可だが「要件を先に固めてから設計→実装」（時系列）は不可。
- **BtoB受発注マッチング**（実体: `beekle-team/oem-backend`＋`oem-client`、「OEMバンク」oem-bank.com、発注元=ノウハウバンク株式会社（代表 桑高裕喜、原料・受託バンクのスピンオフ）、**受託・許諾未取得**）。業種（健康食品/サプリOEM）も特定につながるため伏せ「BtoBマッチング」。決済GW統合は無し（DB管理＋成果報酬）＝「決済実装」と書かない。
- **サブスク課金型マッチング**（実体: `beekle-team/matchin-*`、「性癖マッチン」seiheki-matching.jp、**アダルト/フェチ系・自社事業**、決済ベンダー Alij/アナザーレーン）。**アダルト性は伏せ「マッチングサービス」**（ユーザー指示）。決済＝外部GW差し替え可能ドライバ抽象（実装1ゲートウェイ）。マッチング＝属性ベース相性スコアリング（LLMではない）。ECSオートスケール/ALB/Redis/WAFはterraform実在。**CDP・BigQuery・Databricks on AWS・RFM/コホート/因果分析・神経科学デザインは鶴岡さん申告ベース（コード未確認・git外SaaS）**＝捏造数値なし・貢献ベースで記載。
- **音声認識を用いた語学学習アプリ**（実体: `beekle-team/nihongo-*`＋`julius-plugin`、「Nihongo Dojo」、自社、外部パートナー Caerux 参画）。音声認識=OpenAI Whisper文字起こしはAppSyncに配線済み（実動）。発音採点=Julius強制アライメント+F0の**R&D試作**（固定サンプル採点）＋モバイルはモックUI＝「システムを構築」までで「モバイル組込み・全面出荷」は書かない。
- **AI占いアプリMVP（React Native）**（実体: `beekle-team/gpt-uranai`、「GPT占い館/マダム・ミミ」gpt-uranai.com、自社）。**MVP/プロトタイプ**（11コミット・テスト0・ストアID未設定）＝「配信中」と書かない。RN/Expo＋Cloudflare Workers＋LLM(OpenRouter)。
- **AIマッチングPoC**（実体: `beekle-team/event-matching`、営業デモ、受託PoCとユーザー申告）。LLM(OpenRouter/claude-sonnet-4)で相性0〜100スコア＝AIマッチングは実装確認済み。15コミット/9日＝PoC明記。
