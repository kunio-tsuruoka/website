...
- **コラム本文に `/tools/*` へのリンクを書かない。** 記事内のアクション導線は「相談（/contact）」か「資料DL（/downloads/*）」のみ。
...
# CRO方針: サービス資料はゲート（2026-05-03、ツール関連は上記で上書き済み）
...
- コラム末CTAは記事カテゴリ／スラッグで分岐（`src/lib/column-cta-mapping.ts`）。主動線は常に相談、副動線は資料DLかゼロスタートLP。**ツールは出さない。**
