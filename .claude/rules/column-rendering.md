...
- 2026-07-24 全128記事を監査したところ、**同一 `<ul>`/`<ol>` 内で16字以下(chip)と17字以上(no-chip)が混在するリスト**が30記事・158ラベルで発生していた（「中途半端に紫」の量産）。**全項目が16字以下で一律チップ化しているリストは対比記事の意図的装飾なので触らない**。混在リストだけを対象に、リードラベルのコロンを全項目 `<strong>` 内側へ移して素の太字に統一した。判定は「1つの ul/ol 内に chip対象と非対象が両方あるか」で機械化できる（混在=バグ、一律=意図通り）。ユーザーが1記事を指摘 → 横展開で全体是正、の典型。
